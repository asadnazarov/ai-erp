...
export function renderProposalHtml(p: ProposalInput): string {
  ...
  <div style="font-family: 'Plus Jakarta Sans', system-ui, sans-serif; color:#17161b; max-width:720px; margin:0 auto; padding:32px;">
    ...
    <div style="display:flex; gap:24px; margin-bottom:28px;">
      ...
      <div>
        ...
        <div style="font-size:20px; font-weight:700; margin-top:4px;">${PAYMENT_TYPE_LABEL[p.paymentType] ?? p.paymentType}</div>
      </div>
      ${
    ...
    <div style="background:#f4f3f1; border-radius:14px; padding:20px 24px;">
      <div style="font-size:12px; color:#9a98a0; text-transform:uppercase; letter-spacing:1px; margin-bottom:6px;">Texnik qo‘llab-quvvatlash</div>
      <div style="font-size:18px; font-weight:700;">${SUPPORT_TIER_LABEL[p.supportTier] ?? p.supportTier} tarifi${p.supportMonths ? ` — ${p.supportMonths} oy` : ""}</div>
      <p style="color:#6b6a70; font-size:14px; margin-top:8px;">
        Loyiha topshirilgandan so‘ng ko‘rsatilgan muddat davomida xatoliklarni bepul tuzatamiz va savollaringizga javob beramiz.
      </p>
    </div>
  </div>`
}
